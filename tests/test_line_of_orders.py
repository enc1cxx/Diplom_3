...
class TestRestorePassword:
    ...
    def test_all_time_order_counter(self, driver, user_returns_token):
        ...
        main_page.go_to_order_feed()
        order_feed_page = OrderFeedPage(driver)
        current_count = order_feed_page.text_in_element(
            OrderPageLocators.ALL_TIME_COUNT
        )
        ...
        with allure.step(
            ...
            assert int(current_count) + 1 == int(
                order_feed_page.text_in_element(OrderPageLocators.ALL_TIME_COUNT)
            )
    ...
    def test_today_order_counter(self, driver, user_returns_token):
        ...
        main_page.go_to_order_feed()
        order_feed_page = OrderFeedPage(driver)
        current_count = order_feed_page.text_in_element(OrderPageLocators.TODAY_COUNT)
        create_order(user_returns_token[1])
        with allure.step(
            "Проверяем, что при создании нового заказа счётчик 'Выполнено за сегодня увеличивается'"
        ):
            assert int(current_count) + 1 == int(
                order_feed_page.text_in_element(OrderPageLocators.TODAY_COUNT)
            )
    ...
    def test_today_order_counter__(self, driver, user_returns_token):
        ...
        order_feed_page = OrderFeedPage(driver)
        order_feed_page.wait_invisibility(OrderPageLocators.ALL_ORDERS_DONE)

        with allure.step(
            "Проверяем, что после оформления заказа его номер появляется в разделе 'В работе'"
        ):
            text = order_feed_page.text_in_element(OrderPageLocators.ORDER_IN_PROGRESS)
